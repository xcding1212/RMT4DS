...
StepWiseSVD = function(Y, threshold=NULL, B=1000, level=0.02, 
    ...
    if(is.null(threshold)){
        ...
        ratios = c()
        for(i in 1:B){
            ratios = c(ratios, simulation(c(M,N)))
        }
    
        # using 1-level percentile to get the threshold to pick q
        threshold = quantile(ratios, probs=1-level)
    ...
    q = tail(which(mu_ratio[1:floor(min(n,p)/2)]>threshold), 1)
    ...
    for(i in 1:q){
        ...
        if(sparse==TRUE){
            if(methods=='kmeans'){
                kmeans_u = kmeans(abs(u), 2)
                kmeans_v = kmeans(abs(v), 2)
...
TestRank = function(Y, r1, r0, type=c("1","2"), level=0.1, threshold=NULL, B=5000){
    ...
    simulation = function(x, r1){
        Matrix_sim = matrix(rnorm(x[1]*x[2], mean=0, sd=sqrt(1/x[2])), nrow=x[1], byrow=TRUE)
    ...
    if(type=="1"){
        ...
        if(is.null(threshold)){
            if(r0==(r1-1)){
                b = quantile(rates1[,1], 1-level)
            } else{
                b = quantile(apply(rates1[,1:(r1-r0)],1,max), 1-level) 
            }
        } else{
            b = threshold
        }
    ...
    if(type=="2"){
        ...
        if(is.null(threshold)){
            if(r0==(r1-1)){
                b = quantile(rates2[,1], 1-level)
            } else{
                b = quantile(apply(rates2[,1:(r1-r0)],1,max), 1-level) 
            }
        } else{
            b = threshold
        }
